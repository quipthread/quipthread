...
| Variable | Default | Description |
|---|---|---|
| `DATABASE_URL` | `./data/comments.db` | Path to the local SQLite database file. Self-hosted mode rejects remote `libsql://`, `http://`, and `https://` URLs before opening a driver. |
...
| Variable | Default | Description |
|---|---|---|
| `TURSO_AUTH_TOKEN` | — | Auth token supplied privately to managed Cloud remote tenant connections. It is not appended to persisted database URLs. |
...
| `TURSO_GROUP` | `default` | Existing Turso group for newly provisioned tenant databases. `TURSO_AUTH_TOKEN` must grant access to this group. |
