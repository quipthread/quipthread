...
The backend reads process environment variables, then loads `../.env` and `.env` without overwriting values already set. Docker Compose injects the `.env` file through `env_file`. Keep one configuration source per deployment to avoid conflicting values.

Production builds require `JWT_SECRET`, at least one exact `ALLOWED_ORIGINS` entry, a valid `BASE_URL`, and at least one complete authentication provider. Email authentication requires `SMTP_HOST` and `SMTP_FROM`. Invalid production configuration stops startup.
...
See [Docker backups](/docs/guides/docker/#back-up-sqlite-with-litestream) for setup and restore behavior.
