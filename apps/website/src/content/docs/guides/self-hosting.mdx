...
Use this guide when you need to build Quipthread from source. For most self-hosted installations, [run the published Docker image](/docs/guides/docker/) instead. It includes the migration CLI and does not require Go or Bun on the host.
...
Use `quipthread:local` in place of the published image in the [Docker Compose or docker run example](/docs/guides/docker/). The Docker build installs its own build tools.

## Protect your data

Use the [backup and restore guide](/docs/guides/backups/) for the Docker image. A directly installed Go binary does not start Litestream for you; configure your own SQLite backup process before using it in production.
