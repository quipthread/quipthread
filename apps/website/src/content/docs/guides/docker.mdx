...
The repository ships a production-ready Docker Compose setup that pulls a pre-built image from the GitHub Container Registry. No local build toolchain required.

- **Pre-built image** — `ghcr.io/quipthread/quipthread:latest`, published automatically on every release
...
`docker compose pull` fetches the latest pre-built image. The container starts in seconds — no build step needed on your server.
...
To test locally without a domain or TLS:
...
Then `docker compose up app` and access `http://localhost:8080`.
