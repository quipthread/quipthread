...
// react → preact/compat at runtime so everything works. Cast once here to avoid
// 40+ individual type errors at every JSX child/content prop call site.
// biome-ignore lint/suspicious/noExplicitAny: recharts/Preact JSX type incompatibility; runtime is correct via @astrojs/preact compat
const rc = _recharts as any
const {
  Area,
  AreaChart,
  Bar,
  ...
  return (
    ...
    >
      ...
      <div style={{ fontWeight: 600, color: 'var(--text)' }}>
        {payload[0].value} comment{payload[0].value !== 1 ? 's' : ''}
      </div>
    </div>
  )
}
...
function BarTooltip({ active, payload }: any) {
  ...
  return (
    ...
    >
      <div
        style={{ color: 'var(--muted)', marginBottom: '0.2rem', wordBreak: 'break-all' as const }}
      >
        {name}
      </div>
      <div style={{ fontWeight: 600, color: 'var(--text)' }}>
      ...
      {RANGES.map((r) => (
        <button
          ...
          style={{
            ...
            fontSize: '0.8125rem',
            fontWeight: 500,
            color: value === r.value ? 'var(--text)' : 'var(--muted)',
            transition: 'all 0.1s',
          }}
        >
          {r.label}
              ...
              <ResponsiveContainer width="100%" height={Math.max(120, data.pages.length * 36)}>
                <BarChart
                  data={data.pages.map((p) => ({
                    ...p,
                    label: truncate(p.page_title, labelTrunc),
                  }))}
                  layout="vertical"
          ...
          {data.commenters.length > 0 && (
            <div style={sectionStyle()}>
              ...
              <ResponsiveContainer width="100%" height={Math.max(120, data.commenters.length * 36)}>
                <BarChart
                  data={data.commenters.map((c) => ({
                    ...c,
                    label: truncate(c.display_name, labelTrunc),
                  }))}
                  layout="vertical"
                  margin={{ top: 0, right: 16, left: 8, bottom: 0 }}
                >
            ...
            <>
                        ...
                        >
                          <span
                            style={{
                              ...
                              height: 10,
                              borderRadius: 2,
                              background: STATUS_COLORS[s.status] ?? axisColor,
                              flexShrink: 0,
                            }}
                          />
                          <span
                            style={{ textTransform: 'capitalize' as const, color: 'var(--text)' }}
                          >
                            {s.status}
                          </span>
                          <span
              ...
              {data.peak_hours && (
                <div style={sectionStyle()}>
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <BarChart
                      data={data.peak_hours.map((h) => ({ ...h, label: fmt12h(h.hour) }))}
              ...
              {data.peak_days && (
                <div style={sectionStyle()}>
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <BarChart
                      data={data.peak_days.map((d) => ({ ...d, label: DAY_LABELS[d.day] }))}
                      margin={{ top: 0, right: 8, left: -16, bottom: 0 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" stroke={gridColor} vertical={false} />
                      <XAxis
